...
force_user_message() {
    ...
    # Get accurate connection count via /proc — counts real SSH sessions
    ...
    # Write plain-text ANSI banner (SSH Banner directive reads file as-is to terminal)
    cat <<EOF > "$msg_file"
\033[1;35m═══════════════════════════════════════\033[0m
\033[1;33m▌\033[0m\033[1;36m   ELITE-X SLOWDNS VPN v5.0         \033[0m\033[1;33m▐\033[0m
\033[1;35m═══════════════════════════════════════\033[0m
\033[1;33m USERNAME  \033[0m: \033[1;32m$username\033[0m
\033[0;34m───────────────────────────────────────\033[0m
\033[1;33m EXPIRE    \033[0m: \033[1;31m$expire_date\033[0m
\033[0;34m───────────────────────────────────────\033[0m
\033[1;33m REMAINING \033[0m: \033[1;36m${remaining_days}d + ${remaining_hours}hr + ${remaining_mins}min\033[0m
\033[0;34m───────────────────────────────────────\033[0m
\033[1;33m LIMIT GB  \033[0m: \033[1;32m$bw_display\033[0m
\033[1;33m USAGE GB  \033[0m: \033[1;31m${usage_gb} GB\033[0m
\033[0;34m───────────────────────────────────────\033[0m
\033[1;33m CONNECTION\033[0m: \033[1;35m${current_conn}/${conn_limit}\033[0m
\033[0;34m───────────────────────────────────────\033[0m
\033[1;33m STATUS    \033[0m: \033[1;32m$status_icon $status_text\033[0m
\033[1;35m═══════════════════════════════════════\033[0m
\033[1;32m   Thanks for using ELITE-X VPN        \033[0m
\033[1;35m═══════════════════════════════════════\033[0m
...
# Banner display settings - ensure ANSI colors show correctly
PrintLastLog no
PrintMotd no
SSHCONF
...
# Accurate connection count via /proc — counts real SSH sessions
current_conn=0
...
if [ $remaining_days -le 0 ] && [ $remaining_hours -eq 0 ]; then
    ...
    # ANSI terminal banner — SSH Banner directive shows this file directly
    cat <<EOF > "$MSG_FILE"
\033[1;35m═══════════════════════════════════════\033[0m
\033[1;33m▌\033[0m\033[1;36m   ELITE-X SLOWDNS VPN v5.0         \033[0m\033[1;33m▐\033[0m
\033[1;35m═══════════════════════════════════════\033[0m
 ...
 * Strategy: count sshd child processes owned by user (ppid != 1)
 * This works for both direct SSH and VPN tunnel connections
 */
static int get_conn_count(const char *user) {
    int count = 0;
    struct passwd *upw = getpwnam(user);
    if (!upw) return 0;
    uid_t target_uid = upw->pw_uid;
    ...
    while ((e = readdir(proc))) {
        ...
        /* Read UID first - skip if not our user */
        char sp[256]; snprintf(sp,sizeof(sp),"/proc/%d/status",pid);
        FILE *sf = fopen(sp,"r"); if (!sf) continue;
        char line[256]; uid_t uid = 99999; int ppid_from_status = 0;
        while (fgets(line,sizeof(line),sf)) {
            if (strncmp(line,"Uid:",4)==0) { sscanf(line,"%*s %u",&uid); }
        }
        fclose(sf);
        ...
        /* ppid != 1 means it is a session process, not the listener */
        char stp[256]; snprintf(stp,sizeof(stp),"/proc/%d/stat",pid);
        FILE *stf = fopen(stp,"r"); if (!stf) continue;
        int ppid=0; char sb[1024]={0}; fgets(sb,sizeof(sb),stf);
...
add_user() {
    ...
    # Ensure /bin/sh is in /etc/shells so SSH accepts login
...
list_users() {
    ...
    # Counts ALL session processes owned by non-root users (sshd children)
    declare -A _sess_map
...
show_dashboard() {
    ...
    # Count unique non-root UIDs with active SSH sessions
    declare -A _dash_om
